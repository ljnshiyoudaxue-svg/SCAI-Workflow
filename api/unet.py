...
class Unet(object):
    _defaults = {
        "model_path": "D:/360Downloads/unet_pytorch_main/Log_model/log_InducedFracture/best_epoch_weights.pth",
        "num_classes": 1 + 1,
        "num_channels": 3,
        "backbone": "resnet50",
        "input_shape": [640, 640],
    ...
    def __init__(self, **kwargs):
        ...
        self.__dict__.update(kwargs)  # 关键修改：使用字典直接更新提高效率
    ...
    def generate(self, onnx=False):
        ...
        # 加载权重（新增权重兼容处理）
        try:
            device = torch.device('cpu')  # 先加载到CPU避免显存占用
            state_dict = torch.load(self.model_path, map_location=device)

            # 处理DataParallel保存的权重（关键修改）
        ...
        if not onnx and self.cuda and torch.cuda.is_available():
